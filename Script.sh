...
resolve_record() {
    ...
    for dns in "${DNS_SERVERS[@]}"; do
        timeout "$DNS_TIMEOUT" \
            dig @"$dns" +tries="$DNS_RETRIES" +search +short "$type" "$domain" 2>/dev/null || true
    done
}
...
menuv() {
echo ""
echo -e "\033[1;32mINSTALLING V2RAY MENU...\033[0m"
